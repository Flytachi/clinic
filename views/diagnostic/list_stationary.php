...
$search_array = array(
	"vs.branch_id = $session->branch AND vs.status = 3 AND vs.level = 12 AND v.direction IS NOT NULL AND ( vs.responsible_id = $session->session_id $is_division )",
	"vs.branch_id = $session->branch AND vs.status = 3 AND vs.level = 12 AND v.direction IS NOT NULL AND ( vs.responsible_id = $session->session_id $is_division ) AND (c.id LIKE '%$search%' OR LOWER(CONCAT_WS(' ', c.last_name, c.first_name, c.father_name)) LIKE LOWER('%$search%') OR LOWER(vs.service_name) LIKE LOWER('%$search%') )"
);
