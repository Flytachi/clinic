...
$session->is_auth(5);
is_config("package");
$header = "Мой пакеты услуг";

$tb = new Table($db, "package_services");
$search = $tb->get_serch();
			...
			<div class="content">
				...
				<div class="<?= $classes['card'] ?>">
          			...
          			<div class="<?= $classes['card-header'] ?>">
		              	<h5 class="card-title">Добавить пакет услуг</h5>
		          	</div>

                    <div class="card-body" id="form_card">
                      	<?php (new PackageServicesModel)->form(); ?>
                  	</div>
                ...
                <div class="<?= $classes['card'] ?>">
					...
					<div class="<?= $classes['card-header'] ?>">
	                  	<h5 class="card-title">Список пакетов услуг</h5>
											...
											<td><?= date_f($row->add_date, 1) ?></td>
	                                      	<td class="text-right">
												<div class="list-icons">
													...
													<a onclick="Update('<?= up_url($row->id, 'PackageServicesModel') ?>')" class="list-icons-item text-primary-600"><i class="icon-pencil7"></i></a>
													<a href="<?= del_url($row->id, 'PackageServicesModel') ?>" onclick="return confirm('Вы уверены что хотите удалить пакет?')" class="list-icons-item text-danger-600"><i class="icon-trash"></i></a>
				                                </div>
	                                      	</td>
                              			</tr>
									<?php endforeach; ?>
    ...
    <script type="text/javascript">
		...
		function Change(id, stat = null) {
            ...
            $.ajax({
				...
				data: { table:"package_services", id:id, is_active: stat },
