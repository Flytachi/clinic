					...
					<div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover table-sm">
                                ...
                                <tbody>
                                    <?php
                                    foreach($db->query("SELECT DISTINCT us.id, us.dateBith, vs.route_id FROM users us LEFT JOIN visit vs ON(us.id=vs.user_id) WHERE vs.completed IS NULL AND vs.status = 2 AND vs.direction IS NOT NULL AND vs.parent_id = {$_SESSION['session_id']} AND vs.grant_id IS NOT NULL ORDER BY vs.add_date ASC") as $row) {
                                        ?>
                                        <tr>
