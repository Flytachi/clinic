...
if ($_GET['main']) {
    ...
    if ($_GET['stage'] == 1) {
        ...
        $sql = "SELECT vs.id FROM visit vs WHERE vs.user_id = {$_GET['id']} AND vs.parent_id = {$_SESSION['session_id']} AND vs.route_id = {$_SESSION['session_id']} AND vs.completed IS NULL AND vs.service_id != 1 AND (vs.report_description IS NULL OR vs.report_diagnostic IS NULL OR vs.report_recommendation IS NULL)";
    ...
    if ($_GET['stage'] == 1) {
        $sql = "SELECT vs.id FROM visit vs WHERE vs.user_id = {$_GET['id']} AND vs.parent_id = {$_SESSION['session_id']} AND vs.completed IS NULL AND vs.report_description IS NOT NULL AND vs.report_diagnostic IS NOT NULL AND vs.report_recommendation IS NOT NULL";
    } else if($_GET['stage'] == 2) {
        $sql = "SELECT vs.id FROM visit vs WHERE vs.user_id = {$_GET['id']} AND vs.route_id = {$_SESSION['session_id']} AND vs.completed IS NULL ";
    }
