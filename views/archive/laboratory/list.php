...
$search = $tb->get_serch();
$where_search = array(
	"us.user_level = 15 AND vs.level = 6 AND vs.status = 7", 
	"us.user_level = 15 AND vs.level = 6 AND vs.status = 7 AND (us.id LIKE '%$search%' OR LOWER(CONCAT_WS(' ', us.last_name, us.first_name, us.father_name)) LIKE LOWER('%$search%'))"
);
...
<body>
									...
									<?php foreach ($tb->get_table() as $row): ?>
										<tr>
											...
											<td class="text-center">
												<a href="<?= viv('archive/laboratory/list_visit') ?>?id=<?= $row->id ?>" class="<?= $classes['btn-detail'] ?>"><i class="icon-eye mr-2"></i> Просмотр</a>
											</td>
										</tr>
									<?php endforeach;?>
								</tbody>
	...
	<script type="text/javascript">
		...
		$("#search_input").keyup(function() {
			$.ajax({
				type: "GET",
				url: "<?= ajax('search/archive-laboratory-list') ?>",
				data: {
