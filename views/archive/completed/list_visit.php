						...
						<div class="table-responsive card">
                            <table class="table table-hover table-sm">
                                ...
                                <tbody>
                                    <?php
                                    $i = 1;
                                    foreach($db->query("SELECT vs.id, vs.route_id, vs.direction, vs.laboratory, vs.add_date, vs.completed, sc.name FROM visit vs LEFT JOIN service sc ON(vs.service_id=sc.id) WHERE user_id = {$_GET['id']} AND parent_id = {$_SESSION['session_id']} AND completed IS NOT NULL ORDER BY add_date DESC") as $row) {
										?>
                                        <tr>
                                            <td><?= $i++ ?></td>
											<td>
