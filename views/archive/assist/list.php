...
$tb->set_data("DISTINCT c.id, c.dateBith, c.numberPhone, c.add_date");
$search = $tb->get_serch();

$search_array = array(
	"vs.completed IS NOT NULL AND vs.assist_id = $session->session_id", 
	"vs.completed IS NOT NULL AND vs.assist_id = $session->session_id AND (c.id LIKE '%$search%' OR LOWER(CONCAT_WS(' ', c.last_name, c.first_name, c.father_name)) LIKE LOWER('%$search%'))"
);
$tb->additions("LEFT JOIN visit vs ON(c.id=vs.client_id)")->where_or_serch($search_array)->order_by("c.id DESC")->set_limit(20);
?>
<!DOCTYPE html>
<html lang="en">
                                ...
                                <tbody>
									...
									<?php foreach ($tb->get_table() as $row): ?>
										<tr>
                                            ...
                                            <td><div class="font-weight-semibold"><?= client_name($row->id) ?></div></td>
