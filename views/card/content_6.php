				    ...
				    <div class="card-body">
						...
						<div class="card">
							...
							<div class="card-header header-elements-inline">
								<h5 class="card-title">Диагностика Пациента</h5>
								<?php if ($activity): ?>
									...
									<tbody>
										...
										if ($patient->completed) {
											$sql_table = "SELECT vs.id, vs.parent_id, vs.direction, vs.accept_date, vs.completed, vs.status, sc.name
															...
															WHERE vs.user_id = $patient->id AND vs.diagnostic IS NOT NULL AND (DATE_FORMAT(vs.add_date, '%Y-%m-%d %H:%i') BETWEEN '$patient->add_date' AND '$patient->completed') ORDER BY vs.id DESC";
										} else {
											$sql_table = "SELECT vs.id, vs.parent_id, vs.direction, vs.accept_date, vs.completed, vs.status, sc.name
															FROM visit vs LEFT JOIN service sc ON(vs.service_id=sc.id)
															WHERE vs.user_id = $patient->id AND vs.diagnostic IS NOT NULL AND (DATE_FORMAT(vs.add_date, '%Y-%m-%d %H:%i') BETWEEN '$patient->add_date' AND '$patient->curent_date') ORDER BY vs.id DESC";
										}
										...
										?>
											<tr>
												...
												<td class="text-center">
													<button onclick="Check('<?= viv('doctor/report') ?>?pk=<?= $row['id'] ?>')" class="btn btn-outline-info btn-sm legitRipple"><i class="icon-eye mr-2"></i> Просмотр</button>
												</td>
											</tr>
										<?php
										}
			...
			<div class="modal-dialog modal-lg">
				<div class="modal-content">
					<div class="modal-header bg-info">
						<h6 class="modal-title">Назначить обследование</h6>
