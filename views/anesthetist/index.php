...
$tb->set_data("DISTINCT v.id, vo.patient_id, p.last_name, p.first_name, p.father_name, p.birth_date, vo.grant_id, vr.id 'order'")->additions("LEFT JOIN visits v ON(v.id=vo.visit_id) LEFT JOIN patients p ON(p.id=vo.patient_id) LEFT JOIN visit_orders vr ON (v.id = vr.visit_id)");
$search = $tb->get_serch();
$search_array = array(
	"v.completed IS NULL AND v.is_active IS NOT NULL",
	"v.completed IS NULL AND v.is_active IS NOT NULL AND (p.id LIKE '%$search%' OR LOWER(CONCAT_WS(' ', p.last_name, p.first_name, p.father_name)) LIKE LOWER('%$search%'))"
);
