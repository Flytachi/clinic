...
$search = $tb->getSearch();
$search_array = array(
	"vs.branch_id = $session->branch AND vs.status = 2 AND vs.level = 13",
	"vs.branch_id = $session->branch AND vs.status = 2 AND vs.level = 13 AND (c.id LIKE '%$search%' OR LOWER(CONCAT_WS(' ', c.last_name, c.first_name, c.father_name)) LIKE LOWER('%$search%'))"
);
                                ...
                                <tbody>
									<?php foreach($tb->list(1) as $row): ?>
										<tr id="VisitService_tr_<?= $row->count ?>">
                                            ...
                                            <td>
												<?php foreach($db->query("SELECT id, service_name FROM visit_services WHERE visit_id = $row->id AND status = 2 AND route_id = $row->route_id AND level = 13") as $serv): ?>
