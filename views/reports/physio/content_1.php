			...
			<div class="content">
								...
								<div class="form-group row">
									...
									<div class="col-md-12">
										...
										<select data-placeholder="Выберите специалиста" name="parent_id" id="parent_id" class="<?= $classes['form-select'] ?>" required>
											<option></option>
											<?php foreach($db->query("SELECT * from users WHERE user_level = 12 AND is_active IS NOT NULL") as $row): ?>
				...
				<?php if ($_POST): ?>
					...
					if ( isset($_POST['parent_id']) and $_POST['parent_id'] ){
						$Iam .= " AND vs.parent_id = {$_POST['parent_id']}";
						$parent = $_POST['parent_id'];
					}else{
						$Iam .= " AND vs.parent_id = $session->session_id";
