...
$sql = "SELECT us.id,
            ...
            @sta_service_count_1 := IFNULL(
                (
                    SELECT COUNT(vp.id) FROM visit vs LEFT JOIN visit_price vp ON(vp.visit_id=vs.id) LEFT JOIN service sc ON(sc.id=vs.service_id)
                    WHERE vp.item_type IN (1) AND sc.type = 1 AND vs.direction IS NOT NULL AND vs.parent_id=us.id AND vs.priced_date IS NOT NULL AND (DATE_FORMAT(vs.priced_date, '%Y-%m-%d') BETWEEN \"{$_POST['date_start']}\" AND \"{$_POST['date_end']}\")
                )
                , 0) 'sta_service_count_1',
            @sta_service_count_2 := IFNULL(
                (
                    SELECT COUNT(vp.id) FROM visit vs LEFT JOIN visit_price vp ON(vp.visit_id=vs.id) LEFT JOIN service sc ON(sc.id=vs.service_id)
                    WHERE vp.item_type IN (1) AND sc.type = 2 AND vs.direction IS NOT NULL AND vs.parent_id=us.id AND vs.priced_date IS NOT NULL AND (DATE_FORMAT(vs.priced_date, '%Y-%m-%d') BETWEEN \"{$_POST['date_start']}\" AND \"{$_POST['date_end']}\")
                )
            ...
            @sta_service_count_3 := IFNULL(
                (
                    ...
                    WHERE vp.item_type IN (5) AND vs.direction IS NOT NULL AND vs.parent_id=us.id AND vs.priced_date IS NOT NULL AND (DATE_FORMAT(vs.priced_date, '%Y-%m-%d') BETWEEN \"{$_POST['date_start']}\" AND \"{$_POST['date_end']}\")
                )
            ...
            @sta_grant_visit_count := IFNULL(
                (
                    SELECT COUNT(vs.id) FROM visit vs LEFT JOIN visit_price vp ON(vp.visit_id=vs.id)
                    WHERE vp.item_type IN (101) AND vs.direction IS NOT NULL AND vs.grant_id=us.id AND vs.priced_date IS NOT NULL AND (DATE_FORMAT(vs.priced_date, '%Y-%m-%d') BETWEEN \"{$_POST['date_start']}\" AND \"{$_POST['date_end']}\")
                )
            ...
            @sta_service_route_count := IFNULL(
                (
                    SELECT COUNT(vs.id) FROM visit vs LEFT JOIN visit_price vp ON(vp.visit_id=vs.id)
                    WHERE vs.direction IS NOT NULL AND vs.route_id=us.id AND vs.priced_date IS NOT NULL AND (DATE_FORMAT(vs.priced_date, '%Y-%m-%d') BETWEEN \"{$_POST['date_start']}\" AND \"{$_POST['date_end']}\")
                )
