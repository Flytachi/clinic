					...
					<div class="card-body">
						...
						<form action="" method="post">
							...
							<div class="form-group row">
								...
								<div class="col-md-3">
									<label>Кассир:</label>
									<select class="<?= $classes['form-multiselect'] ?>" data-placeholder="Выбрать кассира" name="priser_id[]" multiple="multiple">
										<?php foreach ($db->query("SELECT DISTINCT pricer_id FROM visit_prices WHERE is_visibility IS NOT NULL AND is_price IS NOT NULL") as $row): ?>
											<option value="<?= $row['pricer_id'] ?>" <?= ( isset($_POST['priser_id']) and in_array($row['pricer_id'], $_POST['priser_id'])) ? "selected" : "" ?>><?= get_full_name($row['pricer_id']) ?></option>
										<?php endforeach; ?>
									</select>
								</div>
