							...
							<div class="form-group row">
								...
								<div class="col-md-3">
									...
									<select class="<?= $classes['form-multiselect'] ?>" data-placeholder="Выбрать кассира" name="pricer_id[]" multiple="multiple">
										<?php foreach ($db->query("SELECT id FROM users WHERE branch_id = $session->branch AND user_level IN (22,23)") as $row): ?>
											<option value="<?= $row['id'] ?>" <?= ( isset($_POST['pricer_id']) and in_array($row['id'], $_POST['pricer_id'])) ? "selected" : "" ?>><?= get_full_name($row['id']) ?></option>
										<?php endforeach; ?>
									</select>
								</div>
				...
				<?php if ($_POST): ?>
					<div class="<?= $classes['card'] ?>">
						...
						<div class="card-body">
							...
							$where1 = "branch_id = $session->branch AND (DATE_FORMAT(price_date, '%Y-%m-%d') BETWEEN '".$_POST['date_start']."' AND '".$_POST['date_end']."')";
							$where2 = "branch_id = $session->branch AND (DATE_FORMAT(add_date, '%Y-%m-%d') BETWEEN '".$_POST['date_start']."' AND '".$_POST['date_end']."')";
							...
							$tb1 = $db->query("SELECT price_date 'date', pricer_id, client_id, price_cash 'cash', price_card 'card', price_transfer 'transfer' FROM visit_service_transactions WHERE $where1 AND is_visibility IS NOT NULL AND is_price IS NOT NULL ORDER BY price_date ASC")->fetchAll();
							$tb2 = $db->query("SELECT add_date 'date', pricer_id, client_id, balance_cash 'cash', balance_card 'card', balance_transfer 'transfer' FROM visit_investments WHERE $where2 ORDER BY add_date ASC")->fetchAll();
							?>
