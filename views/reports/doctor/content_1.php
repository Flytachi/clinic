...
$session->is_auth();
$header = "Отчёт врачей по услугам";
?>
<!DOCTYPE html>
<html lang="en">
	...
	<div class="page-content">
		...
		<div class="content-wrapper">
			...
			<div class="content">
								...
								<div class="form-group row">
									...
									<div class="col-md-6">
										...
										<select data-placeholder="Выберите специалиста" name="responsible_id" id="responsible_id" class="<?= $classes['form-select'] ?>" required>
											<?php foreach($db->query("SELECT * FROM users WHERE branch_id = $session->branch AND user_level = 11") as $row): ?>
												<option value="<?= $row['id'] ?>" data-chained="<?= $row['division_id'] ?>" <?= ( isset($_POST['responsible_id']) and $_POST['responsible_id'] == $row['id']) ? "selected" : "" ?>><?= get_full_name($row['id']) ?></option>
											<?php endforeach; ?>
										</select>
									</div>
				...
				<?php if ($_POST): ?>
					<?php
					$Iam = "vs.branch_id = $session->branch AND vs.level = 11 AND vs.service_id != 1"; 
					if ( isset($_POST['responsible_id']) and $_POST['responsible_id'] ){
						$Iam .= " AND vs.responsible_id = {$_POST['responsible_id']}";
						$parent = $_POST['responsible_id'];
					}else{
						$Iam .= " AND vs.responsible_id = $session->session_id";
					...
					<div class="<?= $classes['card'] ?>">
						...
						<div class="card-body">
							...
							<div class="text-center">
								<h4 class="mb-0 font-weight-semibold"><?= get_full_name($parent) ?></h4>
								<p class="mb-2 text-muted"><?= level_name($parent) ." ". division_name($parent) ?></p>
								<b>Дата: </b><?= $_POST['date'] ?>,
