        ...
        if (d.id == id) {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $(
                        `tr[data-userid=${d.user[0].user_id}][data-parentid=${d.user[0].parent_id}]`
                    ).remove();
                    $(
                        `tr[data-parentid=${d.user[0].parent_id}][data-status=accept_patient]`
                    ).remove();
        ...
        if (d.id == id) {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $("#wew").html(
                        `Новый пациент ${d.queue[0].last_name} - ${d.queue[0].first_name}`
                    );
                    ...
                    $(`#${d.queue[0].parent_id}`)
                        .append(`<tr data-userid="${d.queue[0].user_id}" style="text-transform: uppercase; font-weight: 900; font-size: 250%;" data-parentid="${d.queue[0].parent_id}">
...
function deletNotice(body) {
    ...
    try {
        ...
        count =
            Number($(`span#noticeus`).text()) -
            Number($(`span[data-idchat=${id1}]`).text());
        if (count != 0) {
