...
foreach ($items as $item) {
    if( isset($item->item_name_id) ){
        ...
        if ($item->warehouse_id == "order") {
            ...
            $qty_max = $db->query("SELECT SUM(item_qty) FROM warehouse_custom WHERE warehouse_id = $item->warehouse_id AND item_die_date > CURRENT_DATE() AND item_name_id = $item->item_name_id $m $c")->fetchColumn(); 
