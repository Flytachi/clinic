...
$search = $tb->get_serch();
$search_array = array(
	"vs.status = 3 AND vs.level = 5 AND v.direction IS NOT NULL AND vs.parent_id = $session->session_id",
	"vs.status = 3 AND vs.level = 5 AND v.direction IS NOT NULL AND vs.parent_id = $session->session_id AND (us.id LIKE '%$search%' OR LOWER(CONCAT_WS(' ', us.last_name, us.first_name, us.father_name)) LIKE LOWER('%$search%'))"
);
                    ...
                    <td class="text-center">
                        ...
                        <div class="dropdown-menu dropdown-menu-right" x-placement="bottom-end" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(1153px, 186px, 0px);">
                            ...
                            <?php if(module('module_bypass')): ?>
                                <a href="<?= viv('card/content_7') ?>?pk=<?= $row->id ?>&activity=1" class="dropdown-item"><i class="icon-magazine"></i>Лист назначения</a>
                            <?php endif; ?>
