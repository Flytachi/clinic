...
<?php if( isset($_GET['divisions']) ): ?>
    <?php foreach ($_GET['divisions'] as $divis_pk): ?>
        ...
        <?php if ($ser): ?>
            <?php $sql = "SELECT sc.id, sc.user_level, dv.title, sc.name, sc.type, sc.price from service sc LEFT JOIN division dv ON(dv.id=sc.division_id) WHERE sc.division_id = $divis_pk AND sc.type IN ({$_GET['types']}) AND (LOWER(dv.title) LIKE LOWER('%$ser%') OR LOWER(sc.name) LIKE LOWER('%$ser%') )"; ?>
        <?php else: ?>
