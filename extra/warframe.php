...
if (isset(ini['SECURITY']['GUARD']) and ini['SECURITY']['GUARD']) {
    if (!file_exists(dirname(__DIR__)."/.key")) dieConnection("Key authentication failed.");
    $key = explode("-", zlib_decode(hex2bin(file(dirname(__DIR__)."/.key")[0])) );
    if ( empty(ini['SECURITY']['SERIAL']) or trim($key[0]) !== trim(ini['SECURITY']['SERIAL']) or !(date('Y-m-d H:i:s') >= date('Y-m-d H:i:s', $key[1]) && date('Y-m-d H:i:s') <= date('Y-m-d H:i:s', $key[2])) ) dieConnection("Key authentication failed.");
}
