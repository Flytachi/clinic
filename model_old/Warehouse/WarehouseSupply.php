...
class WarehouseSupplyModel extends ModelOld
{
    ...
    public function warehouse_change($pk)
    {
        ...
        foreach ($db->query("SELECT * FROM $this->_warehouse_item WHERE uniq_key = '{$data['uniq_key']}'") as $item) {
            ...
            if ($obj) $object = Mixin\update($this->_storage, array('item_qty' => $obj['item_qty']+$item['item_qty']), $obj['id']);
            else $object = Mixin\insert($this->_storage, $item);
